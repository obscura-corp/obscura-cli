use crate::crypto::aead::{AeadKey, encrypt_with_key, decrypt_with_key};
...
impl SessionStore {
    ...
    fn save_encrypted(session: &SessionFile) -> ObscuraResult<()> {
        let path = Self::session_path()?;
        let session_key = Self::derive_session_key()?;
        let session_data = serde_json::to_vec(session).map_err(|_| ObscuraError::EncryptionFailed)?;
        ...
        let temp_path = path.with_extension("tmp");
        {
            ...
            file.lock_exclusive().map_err(|_| ObscuraError::FilePermissionError)?;
            file.write_all(&encrypted_data).map_err(|_| ObscuraError::FilePermissionError)?;
            file.sync_all().map_err(|_| ObscuraError::FilePermissionError)?;
    ...
    fn derive_session_key() -> ObscuraResult<AeadKey> {
        // Derive key from system-specific information for additional security
